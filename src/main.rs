...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    info!(
        "LOAD short SET {}",
        &ll.set_state(":shor", sdl1000x::State::Off).await?
    );
    info!(
        "LOAD input SET {}",
        &ll.set_state(":inp", sdl1000x::State::Off).await?
    );
    info!(
        "LOAD sense SET {}",
        &ll.set_state("syst:sens", sdl1000x::State::On).await?
    );

    info!("PWR track 0");
    lp.send("output:track 0").await?;
    info!("PWR ch1 off");
    lp.send("output ch1,off").await?;
    info!("PWR ch2 off");
    lp.send("output ch2,off").await?;
    info!("PWR ch3 off");
    lp.send("output ch3,off").await?;
    info!("PWR wave ch1 off");
    lp.send("output:wave ch1,off").await?;
    info!("PWR wave ch2 off");
    lp.send("output:wave ch2,off").await?;

    info!("PWR ch1 volt SET {}", &lp.set("ch1:volt", 4.250).await?);
    info!("PWR ch1 curr SET {}", &lp.set("ch1:curr", 0.250).await?);
    info!("PWR ch1 SET on");
    lp.send("output ch1,on").await?;
    ...
    info!(
        "LOAD func SET {}",
        &ll.set_func(sdl1000x::Func::Curr).await?
    );
    // 5A or 30A
    ...
    info!(
        "LOAD input SET {}",
        &ll.set_state(":inp", sdl1000x::State::On).await?
    );
    ...
    info!(
        "PWR ch1 volt? {:.3}",
        &lp.meas(spd3303x::Ch::Ch1, spd3303x::Meas::Volt).await?
    );
    info!(
        "PWR ch1 curr? {:.3}",
        &lp.meas(spd3303x::Ch::Ch1, spd3303x::Meas::Curr).await?
    );
    info!(
        "PWR ch1 power? {:.3}",
        &lp.meas(spd3303x::Ch::Ch1, spd3303x::Meas::Pow).await?
    );
    info!("***");
    ...
    info!(
        "LOAD sense SET {}",
        &ll.set_state("syst:sens", sdl1000x::State::Off).await?
    );
